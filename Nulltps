-- // official kyro.dev ui library source code self leak by v2ce
...
-- // cleanups
if game.CoreGui:FindFirstChild("KyroDev") then game.CoreGui:FindFirstChild("KyroDev"):Destroy() end 
...
local function CreateWatermark(title)
	...
	vh_title.Text = title or "KYRO.DEV"
...
function lib:Create(ver, size, hidekey)
    ...
    if UserInputService.TouchEnabled then
        ...
        MobileToggle.Parent = Vice
        MobileToggle.Position = UDim2.new(0.9, -50, 0.1, 0)
        MobileToggle.Size = UDim2.new(0, 50, 0, 50)
        MobileToggle.BackgroundColor3 = Color3.fromRGB(23, 20, 41)
        MobileToggle.BackgroundTransparency = 0.5
        MobileToggle.Image = "rbxassetid://10272150497" -- Using the logo as button
        MobileToggle.ImageColor3 = Color3.fromRGB(107, 89, 222)
	...
	UserInputService.InputBegan:Connect(function(key)
		if key.KeyCode == hidekey or key.KeyCode == Enum.KeyCode.K then
			pcall(function()
	...
	function tabs:Tab(title, AssID, desc1, desc2)
		...
		AllSubTabBtns.Size = UDim2.new(1, -40, 0, 0) -- Hiding SubTab Buttons
        AllSubTabBtns.Visible = false
		...
		function subtabs:SubTab(title, AssID)
			local Left = AllSubPagesFolder:FindFirstChild("Left")
			local Right = AllSubPagesFolder:FindFirstChild("Right")
			...
			function items:CounterLabel(side, text, arrow)
				...
				CounterLabelTitle.TextSize = 10.000
			...
			function items:Bind(side, text, mode, callback)
				...
				BindContainer.Size = UDim2.new(1, 0, 0, 28)
				...
				KeybindKeytext.TextSize = 10.000
			...
			function items:Dropdown(side, text, default, options, cb, multi)
				...
				DropdownSelected.TextSize = 10.000
...
local Main = lib:Create('<font color="rgb(107, 89, 222)">KYRO.DEV</font>',  UDim2.new(0, 750, 0, 750), Enum.KeyCode.RightShift)
local T1 = Main:Tab('HOME', 10063461239, 'Home', '             Welcome to Kyro.Dev')
local T2 = Main:Tab('REACH', 10063461239, 'Reach Options', '             Reach configuration.')
local T_Head = Main:Tab('HEAD REACH', 10063461239, 'Head Reach', '             Head Reach configuration.')
local T_Reacts = Main:Tab('REACTS', 10063461239, 'Reacts', '             Reacts configuration.')
local T4 = Main:Tab('HELPERS', 10191671863, 'Helpers Options', 'Helpers configuration.') 
local T5 = Main:Tab('AIMBOT', 10063472975, 'Aimbot Options', 'Aimbot configuration.') 

-- ========= SUB TABS ===============
local HomeTab = T1:SubTab('Home', 10063461239)
local ReachLegsTab = T2:SubTab('Reach Legs', 10063461239)
local MossingTab = T2:SubTab('Mossing', 10063461239)
local VolleyReachTab = T2:SubTab('Volley Reach', 10063480247)

local HeadReachTab = T_Head:SubTab('Head Reach', 10063461239)
local ReactsTab = T_Reacts:SubTab('Reacts', 10063461239)
local ReactsMiscTab = T_Reacts:SubTab('Misc Reacts', 12501131079)
local HelpersTab = T4:SubTab('Helpers', 10257728029)
local VisualsTab = T4:SubTab('Visuals', 10246075923)
local PlayerTab = T4:SubTab('Player', 10246507314)
local AimbotTab = T5:SubTab('Aimbot', 10063341801)

CreateWatermark(); 

-- ================= HOME TAB =================
HomeTab:Label('Left', '--- CONTRIBUTORS ---')
HomeTab:Label('Left', 'kyro')
HomeTab:Label('Left', 'gonza')
HomeTab:Label('Left', 'kenyah')

HomeTab:Label('Right', '--- LATEST UPDATES ---')
HomeTab:Label('Right', '- Aimbot Modes: Shoot, Pass, Save')
HomeTab:Label('Right', '- Head Reach Visualizer')
HomeTab:Label('Right', '- Removed Ball Reach Tab')
HomeTab:Label('Right', '- Mobile Toggle Button')
HomeTab:Label('Right', '- PC Toggle Keybind (K)')
HomeTab:Label('Right', '- UI Improvements')
...
ReachLegsTab:Label('Left', '--- REACH LEGS ---')
ReachLegsTab:Toggle('Left', 'Enable Reach (Firetouchinterest)', false, function(Value)
    reachEnabled = Value
...
ReachLegsTab:Slider('Left', 'Reach Size', 1, 1, 15, 0.1, function(val)
    reachDistance = tonumber(val)
end)

ReachLegsTab:Textbox('Left', 'Legs Size (R6)', 'Reach', false, function(Value)
    local v = tonumber(Value) or 1
    local lp = game.Players.LocalPlayer
    if lp.Character then
        if lp.Character:FindFirstChild("Right Leg") then
            lp.Character["Right Leg"].Size = Vector3.new(v, 2, v)
            lp.Character["Left Leg"].Size = Vector3.new(v, 2, v)
            lp.Character["Right Leg"].CanCollide = false
            lp.Character["Left Leg"].CanCollide = false
        end
        if lp.Character:FindFirstChild("HumanoidRootPart") then
            lp.Character.HumanoidRootPart.Size = Vector3.new(v,2,v)
            lp.Character.HumanoidRootPart.CanCollide = false
        end
    end
end)

ReachLegsTab:Button('Left', 'Fake legs (Appear Normal)', function()
    local player = game.Players.LocalPlayer
...
local headReachDistance = 1
local headReachConnection
local headReachVisualizer = nil
local headReachTransparency = 0.5
local headReachShowVisuals = false

local function updateHeadVisuals()
    if headReachShowVisuals and headReachEnabled then
        if not headReachVisualizer then
            headReachVisualizer = Instance.new("Part")
            headReachVisualizer.Name = "HeadReachVisualizer"
            headReachVisualizer.Anchored = true
            headReachVisualizer.CanCollide = false
            headReachVisualizer.Shape = Enum.PartType.Ball
            headReachVisualizer.Material = Enum.Material.ForceField
            headReachVisualizer.Color = Color3.fromRGB(255, 0, 0)
            headReachVisualizer.Parent = workspace
        end
        headReachVisualizer.Size = Vector3.new(headReachDistance*2, headReachDistance*2, headReachDistance*2)
        headReachVisualizer.Transparency = headReachTransparency
    else
        if headReachVisualizer then
            headReachVisualizer:Destroy()
            headReachVisualizer = nil
        end
    end
end

HeadReachTab:Label('Left', '--- HEAD REACH ---')
HeadReachTab:Toggle('Left', 'Enable Head Reach', false, function(Value)
    headReachEnabled = Value
    updateHeadVisuals()
    
    if not Value and headReachConnection then
        headReachConnection:Disconnect()
        headReachConnection = nil
    end

    if Value then
        if headReachConnection then headReachConnection:Disconnect() end
        headReachConnection = game:GetService("RunService").RenderStepped:Connect(function()
            local player = game.Players.LocalPlayer
            local character = player and player.Character
            ...
            if headReachVisualizer then
                headReachVisualizer.Position = head.Position
            end
            ...
            if distance <= headReachDistance then
...
HeadReachTab:Slider('Left', 'Head Reach Size', 1, 1, 15, 0.1, function(val)
    headReachDistance = tonumber(val)
    updateHeadVisuals()
end)

HeadReachTab:Toggle('Left', 'Show Visualizer', false, function(state)
    headReachShowVisuals = state
    updateHeadVisuals()
end)

HeadReachTab:Slider('Left', 'Visual Transparency', 0, 0.5, 1, 0.1, function(val)
    headReachTransparency = val
    if headReachVisualizer then headReachVisualizer.Transparency = val end
end)

-- ================= REACTS TAB =================
ReactsTab:Label('Left', '--- REACT CONFIGS ---')
ReactsTab:Toggle('Left', 'No Ball Delay', false, function(state)
    -- Placeholder for No Ball Delay logic
    -- Typically involves network replication improvements or removing physics throttling
    if state then
...
ReactsTab:Toggle('Left', 'Better React', false, function(state)
    -- Placeholder for Better React
    if state then
         local tps = workspace:FindFirstChild("TPSSystem") and workspace.TPSSystem:FindFirstChild("TPS")
         if tps then tps.CustomPhysicalProperties = PhysicalProperties.new(0.5, 0.3, 0.5, 1, 1) end
    end
end)

ReactsTab:Toggle('Left', 'OP React', false, function(state)
    -- Placeholder for OP React
    if state then
         local tps = workspace:FindFirstChild("TPSSystem") and workspace.TPSSystem:FindFirstChild("TPS")
         if tps then tps.CustomPhysicalProperties = PhysicalProperties.new(100, 100, 100, 100, 100) end
    end
end)

ReactsTab:Button('Left', 'Alz React', function()
    if workspace.TPSSystem:FindFirstChild("TPS") then
        workspace.TPSSystem.TPS.Velocity = Vector3.new(100, 100, 100)
    end
end)

ReactsTab:Button('Left', 'Foxtede React', function()
    if workspace.TPSSystem:FindFirstChild("TPS") then
        workspace.TPSSystem.TPS.Velocity = Vector3.new(110, 110, 110)
    end
end)

ReactsTab:Button('Left', 'Goalkeeper React', function()
    local gkActions = {"SaveRA", "SaveLA", "SaveRL", "SaveLL", "SaveT", "Tackle", "Header"}
    for _, action in ipairs(gkActions) do
        -- Implementation
    end
end)
local mossHeadReachEnabled = false
local mossHeadReachSize = Vector3.new(1, 1.5, 1)
local mossHeadTransparency = 0.5
local mossHeadOffset = Vector3.new(0, 0, 0)
local mossHeadBoxPart = nil
local mossHeadConnection = nil
local mossUsePercentage = false
local mossPercentageValue = 1

local function updateMossHeadBox()
    if mossHeadBoxPart then mossHeadBoxPart:Destroy() end
    mossHeadBoxPart = Instance.new("Part")
    mossHeadBoxPart.Size = mossHeadReachSize
    mossHeadBoxPart.Transparency = mossHeadTransparency
    mossHeadBoxPart.Anchored = true
    mossHeadBoxPart.CanCollide = false
    mossHeadBoxPart.Color = Color3.fromRGB(255, 105, 180)
    mossHeadBoxPart.Material = Enum.Material.Neon
    mossHeadBoxPart.Name = "MossHeadReachBox"
    mossHeadBoxPart.Parent = workspace
end

local function startMossHeadReach()
    if not mossHeadReachEnabled then return end
    if mossHeadConnection then mossHeadConnection:Disconnect() end
    updateMossHeadBox()

    mossHeadConnection = game:GetService("RunService").RenderStepped:Connect(function()
        local player = game.Players.LocalPlayer
        ...
        if mossHeadBoxPart then
            mossHeadBoxPart.CFrame = head.CFrame * CFrame.new(mossHeadOffset)
            
            local boxCFrame = mossHeadBoxPart.CFrame
            local boxSize = mossHeadBoxPart.Size
...
local function stopMossHeadReach()
    if mossHeadConnection then mossHeadConnection:Disconnect() end
    if mossHeadBoxPart then mossHeadBoxPart:Destroy() end
end

local function getMossAdjustedSize(x, y, z)
    return Vector3.new(x, y, z)
end

MossingTab:Label('Left', '--- MOSSING & HEAD REACH ---')
MossingTab:Toggle('Left', 'Enable Moss & Head Reach', false, function(state)
    mossHeadReachEnabled = state
    if state then startMossHeadReach() else stopMossHeadReach() end
end)

MossingTab:Slider('Left', 'Reach X', 0, 1, 50, 1, function(val)
    mossHeadReachSize = Vector3.new(val, mossHeadReachSize.Y, mossHeadReachSize.Z)
    if mossHeadReachEnabled then updateMossHeadBox() end
end)

MossingTab:Slider('Left', 'Reach Y', 0, 1.5, 50, 1, function(val)
    mossHeadReachSize = Vector3.new(mossHeadReachSize.X, val, mossHeadReachSize.Z)
    mossHeadOffset = Vector3.new(mossHeadOffset.X, val / 2.5, mossHeadOffset.Z)
    if mossHeadReachEnabled then updateMossHeadBox() end
end)

MossingTab:Slider('Left', 'Reach Z', 0, 1, 50, 1, function(val)
    mossHeadReachSize = Vector3.new(mossHeadReachSize.X, mossHeadReachSize.Y, val)
    if mossHeadReachEnabled then updateMossHeadBox() end
end)

MossingTab:Toggle('Left', 'Appear Normal', false, function(v)
    mossHeadTransparency = v and 1 or 0.5
    if mossHeadReachEnabled and mossHeadBoxPart then
        mossHeadBoxPart.Transparency = mossHeadTransparency
    end
end)

MossingTab:Label('Right', '--- PERCENTAGE ---')

MossingTab:Toggle('Right', 'Enable Percentage', false, function(state)
    mossUsePercentage = state
    if state then
        local size = mossPercentageValue * 0.1
        mossHeadReachSize = getMossAdjustedSize(size, mossHeadReachSize.Y, size)
        updateMossHeadBox()
        startMossHeadReach()
    end
end)

MossingTab:Slider('Right', 'Percentage Value', 1, 1, 100, 1, function(value)
    mossPercentageValue = value
    if mossUsePercentage then
...
MossingTab:Label('Right', '--- MOSS CONFIGS ---')

-- Replace the button loop with a single slider for cleaner UI
MossingTab:Slider('Right', 'Moss Power %', 25, 25, 100, 25, function(percent)
...
local volleyReachEnabled = false
local volleyReachDistance = 10
local volleyReachConnection = nil
...
local function startVolleyReachLoop()
    if volleyReachConnection then
        volleyReachConnection:Disconnect()
        volleyReachConnection = nil
    end

    if not volleyReachEnabled or not volleyPart then return end

    volleyReachConnection = game:GetService("RunService").RenderStepped:Connect(function()
        ...
        local distance = (rootPart.Position - tps.Position).Magnitude
        if distance <= volleyReachDistance then
...
VolleyReachTab:Label('Left', '--- VOLLEY REACH ---')
VolleyReachTab:Button('Left', 'Blue Team', function()
...
VolleyReachTab:Button('Left', 'Red Team', function()
...
VolleyReachTab:Toggle('Left', 'Enable Volley Reach', false, function(state)
    volleyReachEnabled = state
    startVolleyReachLoop()
end)

VolleyReachTab:Slider('Left', 'Volley Reach Size', 1, 0, 15, 1, function(value)
    volleyReachDistance = value
end)

-- ================= REACTS TAB =================
-- Reacts Buttons moved to main tab definition


ReactsMiscTab:Toggle('Right', 'Enable Ball Collision', false, function(state)
    if state then
...
-- ================= HELPERS TAB =================
HelpersTab:Toggle('Left', 'ZZZZ Helper', false, function(state)
    if state then
...
HelpersTab:Toggle('Left', 'Air Dribble Helper', false, function(state)
    if not state and workspace:FindFirstChild("TPS_Air") then
        workspace.TPS_Air:Destroy()
    end
end)

HelpersTab:Slider('Left', 'Air Dribble Size', 1, 1, 15, 1, function(val)
...
HelpersTab:Label('Right', '--- VISUALS ---')
HelpersTab:Toggle('Right', 'Ball ESP', false, function(state)
    if state then
        ...
        hl.Parent = workspace.TPSSystem.TPS
    else
        if workspace.TPSSystem.TPS:FindFirstChild("BallHighlight") then
            workspace.TPSSystem.TPS.BallHighlight:Destroy()
        end
    end
end)
...
-- ================= AIMBOT TAB =================
local aimbotMode = nil -- "Shoot", "Pass", "Save"
local aimbotTargetPos = Vector3.new(0, 14, 157) -- Default Shoot Target
local aimbotPassTargetPlayer = nil
local aimbotGoalName = "Fix2" -- Default to Red Goal (Fix2) if we are Blue
...
local function updateAimbotState()
    laser.Transparency = aimbotMode and 0.4 or 1
end

game:GetService("RunService"):BindToRenderStep("L1ghtAimbotLoop", Enum.RenderPriority.Camera.Value + 1, function()
    local char = game.Players.LocalPlayer.Character
    local hrp = char and char:FindFirstChild("HumanoidRootPart")
    local torso = char and (char:FindFirstChild("UpperTorso") or char:FindFirstChild("Torso"))
    
    if aimbotMode and hrp and torso then
        local target = aimbotTargetPos -- Default

        if aimbotMode == "Pass" then 
            if aimbotPassTargetPlayer then
                local p = game.Players:FindFirstChild(aimbotPassTargetPlayer)
                if p and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
                    target = p.Character.HumanoidRootPart.Position
                end
            end
        elseif aimbotMode == "Save" then 
             local ball = workspace:FindFirstChild("TPSSystem") and workspace.TPSSystem:FindFirstChild("TPS")
             if ball then
                 target = ball.Position
             end
        elseif aimbotMode == "Shoot" then
            local goal = workspace.GKSystem:FindFirstChild(aimbotGoalName)
            if goal then
        ...
        local lookTarget = Vector3.new(target.X, hrpPos.Y, target.Z)
        ...
        laser.CFrame = CFrame.lookAt(startPos, target) * CFrame.new(0, 0, -distance/2)
    end
end)

-- SHOOT CONFIG
AimbotTab:Label('Left', '--- SHOOT CONFIG ---')
AimbotTab:Toggle('Left', 'Enable Shoot', false, function(state)
    aimbotMode = state and "Shoot" or nil
    updateAimbotState()
end)
AimbotTab:Bind('Left', 'Shoot Key', 'Toggle', function()
    aimbotMode = (aimbotMode == "Shoot") and nil or "Shoot"
    updateAimbotState()
end)
AimbotTab:Button('Left', 'Target: Red Goal', function()
    aimbotGoalName = "Fix2"
end)
AimbotTab:Button('Left', 'Target: Blue Goal', function()
    aimbotGoalName = "Fix1"
end)

-- PASS CONFIG
AimbotTab:Label('Right', '--- PASS CONFIG ---')
local playerList = {}
...
AimbotTab:Dropdown('Right', 'Select Player', playerList[1] or "None", playerList, function(val)
    aimbotPassTargetPlayer = val
end)

AimbotTab:Toggle('Right', 'Enable Pass', false, function(state)
    aimbotMode = state and "Pass" or nil
    updateAimbotState()
end)
AimbotTab:Bind('Right', 'Pass Key', 'Toggle', function()
    aimbotMode = (aimbotMode == "Pass") and nil or "Pass"
    updateAimbotState()
end)

-- SAVE CONFIG
AimbotTab:Label('Right', '--- SAVE CONFIG ---')
AimbotTab:Toggle('Right', 'Enable Save (Ball)', false, function(state)
    aimbotMode = state and "Save" or nil
    updateAimbotState()
end)
AimbotTab:Bind('Right', 'Save Key', 'Toggle', function()
    aimbotMode = (aimbotMode == "Save") and nil or "Save"
    updateAimbotState()
end)

-- ================= GAMEPASS TAB REMOVED =================
getgenv().library = lib
return lib
